...
**Topics:** 自然语言投研：EDB 数据服务 + 扣子（Coze）

---

## 自然语言投研：EDB 数据服务 + 扣子（Coze）

**URL:** https://doc.shinnytech.com/tqsdk/latest/ai_editor/nl_research.html
...
本主题介绍如何把 天勤 EDB 数据服务（HTTP） 与 扣子（Coze） 结合起来，用自然语言完成“取数 → 验证想法 → 回测 → 产出图表/报告/代码”的投研闭环。
...
SKILL 添加地址：天勤 EDB 数据 SKILL

添加后即可在对话中直接让扣子获取历史数据，并继续完成想法验证或策略回测。

扣子 SKILL 页面示意（点击“使用”完成添加）。

行情历史服务（CSV 流式）：文档：行情历史服务

Token 服务（JWT）：文档：Token 服务

EDB 指标服务（JSON）：文档：非价量数据服务
